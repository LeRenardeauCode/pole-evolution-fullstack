...
export const getMedias = asyncHandler(async (req, res) => {
  const { categorie, type, estPublic, estModere, page = 1, limit = 20 } = req.query;
...
export const createMedia = asyncHandler(async (req, res) => {
  ...
  const mediaData = {
    ...
    type,
    url: req.file.path, 
    urlMiniature: req.file.thumbnail || null,
    nomFichier: req.file.originalname,
    tailleFichier: req.file.size,
    formatFichier: req.file.mimetype.split('/')[1].toUpperCase(),
  ...
  if (type === 'image' && req.file.width) {
    mediaData.largeur = req.file.width;
    mediaData.hauteur = req.file.height;
  }

  if (type === 'video' && req.file.duration) {
    mediaData.duree = req.file.duration;
  }
...
export const updateMedia = asyncHandler(async (req, res) => {
  ...
  if (req.file) {
    ...
    if (media.urlMiniature) {
      await deleteFile(media.urlMiniature);
    }
    
    req.body.url = req.file.path;
    req.body.urlMiniature = req.file.thumbnail || null;
    req.body.nomFichier = req.file.originalname;
    req.body.tailleFichier = req.file.size;
    req.body.formatFichier = req.file.mimetype.split('/')[1].toUpperCase();

    if (req.file.width) {
      req.body.largeur = req.file.width;
      req.body.hauteur = req.file.height;
    }
    if (req.file.duration) {
      req.body.duree = req.file.duration;
    }
  }
  ...
  if (req.body.estPublic !== undefined) {
    req.body.estPublic = req.body.estPublic === 'true' || req.body.estPublic === true;
  }
  if (req.body.estALaUne !== undefined) {
    req.body.estALaUne = req.body.estALaUne === 'true' || req.body.estALaUne === true;
  }
