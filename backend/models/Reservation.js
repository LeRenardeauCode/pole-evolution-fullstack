...
const reservationSchema = new mongoose.Schema({
  ...
  paiement: {
    statut: {
      type: String,
      enum: {
        values: ['en_attente', 'paye', 'echoue', 'rembourse'],
        message: 'Statut paiement invalide'
      },
      default: 'paye'
      // Si forfait utilisé, statut = 'paye' directement
    },
    montant: {
      type: Number,
      required: false,
      min: [0, 'Le montant ne peut pas être négatif']
      // Montant payé (0 si forfait, ou prix unitaire si paiement à l'unité)
    },
    methodePaiement: {
      type: String,
      enum: {
        values: ['forfait', 'carte_bancaire', 'especes', 'cheque', 'virement'],
    ...
    referenceTransaction: {
      ...
      // ID transaction Stripe/PayPal si paiement en ligne
    }
  },
...
reservationSchema.virtual('peutEtreAnnulee').get(function() {
  if (this.statut === 'annulee' || this.statut === 'present' || this.statut === 'absent') {
    return false;
  }

  // Note: nécessite un populate('cours') avant
  if (this.cours && this.cours.dateDebut) {
    const maintenant = new Date();
    const delaiAnnulation = 24; // heures (à récupérer des paramètres)
    const dateLimit = new Date(this.cours.dateDebut.getTime() - delaiAnnulation * 60 * 60 * 1000);
...
reservationSchema.methods.annuler = async function(raison, annulePar = 'client') {
  const Cours = mongoose.model('Cours');
  ...
  // Rembourser la séance au forfait si applicable
  if (this.forfait && annulePar === 'client') {
...
reservationSchema.methods.envoyerRappel = async function() {
  // TODO: Intégrer service d'envoi email/SMS (Phase 5)
...
reservationSchema.statics.getHistoriqueUtilisateur = function(utilisateurId, limite = 20) {
...
reservationSchema.statics.compterParStatut = async function() {
  ...
  return {
    ...
    tauxPresence: presents + absents > 0 
...
reservationSchema.statics.aDejaReserve = async function(utilisateurId, coursId) {
...
reservationSchema.statics.getStatistiques = async function(dateDebut, dateFin) {
  const query = {};
  ...
  const confirmees = await this.countDocuments({ ...query, statut: 'confirmee' });
  ...
  const revenus = await this.aggregate([
    { $match: { ...query, 'paiement.statut': 'paye' } },
