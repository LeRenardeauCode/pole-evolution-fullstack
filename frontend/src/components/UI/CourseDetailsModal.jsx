...
const CourseDetailsModal = ({ open, onClose, cours, onReservationSuccess }) => {
  ...
  const peutReserver = cours.statut !== 'annule' && 
            ...
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
              ...
              <Box>
                ...
                <Typography variant="body1" fontWeight="bold">
                  {format(new Date(cours.dateDebut), "EEEE d MMMM yyyy", { locale: fr })}
                </Typography>
              </Box>
            </Box>
          ...
          <Box>
            ...
            {placesDisponibles > 0 ? (
              <Alert severity="success" icon={<CheckCircle />}>
                <strong>{placesDisponibles} place{placesDisponibles > 1 ? 's' : ''} disponible{placesDisponibles > 1 ? 's' : ''}</strong>
              </Alert>
            ) : (
